...
def school_status(profile):
    ...
    df = pd.read_csv("vars_mini.csv")  
    millie_int_schools = df['School Name'].tolist()
    ...
    for each in schools_details:
        ...
        for school in millie_int_schools:
            if school in each:
                result['target'] = True
                result['type'] = 'Millie school name present in profile'
    ...
    for each in schools:
        ...
        for school in millie_int_schools:
            if each == school:
                result['target'] = True
                result['type'] = 'Complete match in Millie school database' 
